...
// Note: Be sure to update the errmsg_xstrings and errmsg_indices arrays£¬so that the error_message function 
// will be able to report the new error code.

// No error; generally the return code should be zero
#define	ENONE		 	 0
// Unix/Posix-compatible codes...
#define	EPERM			 1	// FFFF Operation not permitted (this is not for user access control, see EACCES for that)
#define	ENOENT			 2	// FFFE No such file or directory
#define	EIO			 	 5	// FFFB I/O error
#define	ENXIO			 6	// FFFA No such device or address
#define	E2BIG			 7	// FFF9 Arg list too long, or something else is too big like a string parameter
...
#define  EUNFLUSHABLE	310	// FECA Cannot flush enough entries from cache to perform next read.  There are pending dirty
              				// cache entries from a previous boot.  Register all devices and this may go away.  If not,
                     		// there are dirty entries for a removable medium, which is not mounted.  In this case,
                 			// call fatwtc_flushdev with the unregister flag.
#define  EMISMATCH		311	// FEC9 Parameter mismatch when registering a device. The device had outstanding cache entries 
							// from previous boot, but the caller is attempting to change the cusize or removable status.
#define  EDEVNOTREG		312	// FEC8 Internal error: device not registered when _fatwtc_devwrite called.
#define  EPARTIALWRITE	313	// FEC7 Internal error: not writing full physical sector in _fatwtc_devwrite.
#define  EJOVERFLOW		314	// FEC6 Rollback journal overflow.  Transaction requires too much data to be stored.  
							// Either increase FAT_MAXRJ in the BIOS, or review calling code to make sure transactions are
                      		// terminated at the right time and do not journal unnecessary data.
#define  ETRANSOPEN		315	// FEC5 fatrj_transtart() called with transaction already open.
#define  EBROKENTIE		316	// FEC4 Internal error: a tied cache group is in an inconsistent state.
#define  ETRANSNOTOPEN	317	// FEC3 fatrj_setchk() called without transaction being open.
#define  ECMCONFLICT 	318	// FEC2 Transaction cannot contain both checkpoint and marker data.
#define  EFSTATE		319	// FEC1 File is in an invalid state.  Probably because the FATfile structure was not zero 
							// when opened for the first time.
#define  EPSTATE		320	// FEC0 Partition is in an invalid state.  This occurs if you are trying to delete a file 
							// when another file is being allocated, or vice versa
...
#define NETERR_NOHOST_ARP				401	// FE6F Local host not reachable (ARP could not resolve)
#define NETERR_NOHOST_RTE				402	// FE6E Host not reachable (Router not resolved)
...
#define NETERR_PROTOCOL					410	// FE66 Protocol error e.g. bad ack number for TCP
...
#define NETERR_NOTDIGIT					501	// FE0B aton2() encountered non-digit in IP address
#define NETERR_TOOBIG					502	// FE0A aton2() encountered decimal field outside 0-255
#define NETERR_BADDELIM					503	// FE09 aton2() encountered delimiter not '.' or ','.
...
#define _SYS_NO_HANDLES					604	// FDA4 no handles available. Close something.
#define _SYS_STACK_LIMIT_VIOLATION		605	// FDA3 Stack limit violation occurred in user mode.
#define _SYS_SYSTEM_MODE_VIOLATION		606	// FDA2 Processor attempted to execute system code while running in user mode.
#define _SYS_WRITE_PROTECT_VIOLATION	607	// FDA1 Write protection violation occurred in user mode.
...
#define SSL_READ_BLOCK_CIPHER_ERROR	938 // Received record did not have length which was multiple of cipher block size
#define SSL_READ_PROTOCOL_ERROR		939 // Protocol error in received record e.g. the record contents were shorter than
										// expected for the given record type, or app data received when handshake incomplete
#define SSL_WRITE_REC_TOO_BIG		940 // Record too large to fit in write buffer
#define SSL_WRITE_BLOCK_CIPHER_ERROR 941	// Attempted to write record which did not have length
...
//----------
#define SSL_CERT_CODE_BASE				950 // Not an error, just start of following block of codes, which are based on
...
#define SSL_BAD_STATE_CLI_HELLO     960 // Inappropriate state to be attempting to send client hello.  State must be
										// initial (SSL_STATE_LISTEN) or done (SSL_STATE_DONE).

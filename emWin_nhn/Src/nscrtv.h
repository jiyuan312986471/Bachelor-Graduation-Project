...
typedef struct {
   __aProperty * pPtr;
   S16 heCommonLogIndex;
   snmp_oid  heCommonLogOid;
   int heCommonLogValue;
   int heCommonLogState;
   unsigned char heCommonLogTime[8];
   char heCommonLogText[32]; 
   word Index[6];		  //save index just inserted for used to delete,for SCTE heCommonLog there're 5 leaves
}__scteHeCommonAlarmLogTable;
...
#define MAX_AP_ITEMS 15  //原来是12，SCTE平台电源模块有15个模拟属性
...
typedef struct {
	...
	snmp_oid devOid;
	U16 isPowerOff; //1: Power Off 0:Power On

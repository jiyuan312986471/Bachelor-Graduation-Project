...
extern volatile int tscPressure;
...
static void _cbBkWindow(WM_MESSAGE* pMsg) {
  ...
  switch (pMsg->MsgId) {
    ...
    case WM_PAINT:
      ...
      break;
	case WM_TOUCH:
		pid_StatePtr = (GUI_PID_STATE*)pMsg->Data.p;
		if (pid_StatePtr->Pressed==1) {
			GUI_DispDecAt(tscPressure, 300, 0, 10);
		}
		break;
    default:
      WM_DefaultProc(pMsg);

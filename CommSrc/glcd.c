...
extern unsigned short dispBuffer[];
#define LCD_VRAM_BASE_ADDR dispBuffer
#define LCD_CURSOR_BASE_ADDR 	((unsigned long)0x20088800)

//unsigned short dispBuffer[C_GLCD_H_SIZE*C_GLCD_V_SIZE] __attribute__((at(LCD_VRAM_BASE_ADDR)));
//unsigned short dispBuffer[C_GLCD_H_SIZE*C_GLCD_V_SIZE] __attribute__((section("RW_RAM1"))) ;
...
{
	uint32_t i ;
   	uint32_t *pSrc, *pDst = (uint32_t *)LCD_CURSOR_BASE_ADDR;
   
   	pDst += cursor*64;
	pSrc = (uint32_t*)pCursor->pCursorData;
   	for(i = 0; i < pCursor->H_Size/4*pCursor->V_Size/sizeof(uint32_t) ; i++) {
		*pDst++ = *pSrc++;
	}
}
